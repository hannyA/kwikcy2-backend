...
exports.handler = (event, context, callback) => {
 ...
 console.log('secretKey: ' + secretKey);
    ...
    function editComment(albumOwnerGuid, albumId, commenterGuid, commenterId, comment) {

        connection.query({
            sql     : "UPDATE album_comments SET comment = ? WHERE guid = ? AND album_id = ? AND commenter_guid AND comment_id = ?",
            values  : [ comment, albumOwnerGuid, albumId, commenterGuid, commenterId]
        },
        function (err, results, fields) {
    ...
    function deleteComment( albumOwnerGuid, albumId, commenterGuid, commenterId) {
    ...
    // What happens when we flag comment. Does user get it? Or does the team?
    
    function flagComment(albumOwnerGuid, albumId, commenterGuid, commenterId, flaggerGuid) {

        var sql = 'INSERT INTO flag_comment SET `album_owner` = ?, `album_id` = ?, `commenter_guid` = ?, `comment_id` = ?, flagger_guid = ?';
        

        connection.query({
            sql     : sql,
            values  : [ albumOwnerGuid, albumId, commenterGuid, commenterId, flaggerGuid]
        },
        function (err, results, fields) {
